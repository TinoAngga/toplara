                                            ...
                                            <div class="order-total table-responsive ">
                                                <table class="table table-borderless text-left">
                                                    <tbody>
                                                        ...
                                                        <tr class="text-white">
                                                            <td>Tanggal</td>
                                                            <td>: {{ format_datetime($deposit->created_at) }} WIB</td>
                                                        </tr>
                                                        <tr class="text-white">
                                                            <td>Harga</td>
                                                            <td>:
                                                                {{ 'Rp ' . currency($deposit->amount - $deposit->fee - $deposit->unique_code) }}
                                                            </td>
                                                        </tr>
                                                        <tr class="text-white">
                                                            <td>Tax</td>
                                                            <td>: {{ 'Rp ' . currency($deposit->fee) }}</td>
                                                        </tr>
                                                        <tr class="text-white">
                                                            <td>Biaya Admin</td>
                        ...
                        <div class="invoice-address">
                            <div class="col-sm-12">
                                <div class="order-note mb-2">
                                    <p class="mb-3 mt-3 text-end">{!! isPaidText($deposit->is_paid) !!}</p>
                                    <div class="order-total table-responsive ">
                                        <table class="table table-borderless text-left">
                                            <tbody>
                                                <tr class="text-white">
                                                    <td>Metode Pembayaran</td>
                                                    <td>{{ strtoupper($deposit->payment->name) }}</td>
                                                </tr>
                                                <tr class="text-white">
                                                    <td>Catatan Pembayaran</td>
                                                    <td>
                                                    @if ($deposit->payment->payment_gateway == 'tripay')
                                                        @php
                                                            $paymentCheckout = json_decode($deposit->payment_gateway_request_response)
                                                        @endphp
                                                        ...
                                                        @else
                                                            -
                                                        @endif
                                                    @elseif ($deposit->payment->payment_gateway == 'xendit' AND
                                                        $deposit->payment->payment_gateway_code === 'qris')
                                                        @php
                                                        $paymentCheckout = json_decode($deposit->payment_gateway_request_response)
                                                        @endphp
                                                        @if ($deposit->is_paid == 0 AND $paymentCheckout)
                                                            {!! convertString($deposit->payment->information, $deposit->amount) !!}
                                                            <br />
                                                        ...
                                                        @endif
                                                    @elseif ($deposit->payment->payment_gateway == 'paydisini')
                                                        @php
                                                            $paymentCheckout = json_decode($deposit->payment_gateway_request_response)
                                                        @endphp
                                                        @if ($deposit->is_paid == 0 AND $paymentCheckout)
                                                            @if (preg_match('/qris/i', $deposit->payment->name))
                                                                {!! convertString($deposit->payment->information, $deposit->amount) !!}
                                                                <br />
                                                                <p style="margin-bottom: -10px;">SCAN QR CODE DI BAWAH INI!</p>
                                                                <br />
                                                                <img src="<?= $paymentCheckout->data->qrcode_url ?>" style="height: 250px; width: 250px">
                                                            @elseif(preg_match('/virtual account/i', $deposit->payment->name))
                                                                {!! convertString($deposit->payment->information, $deposit->amount) !!}
                                                            ...
                                                            @elseif($deposit->payment->type == 'convience_store')
                                                                {!! convertString($deposit->payment->information, $deposit->amount) !!}
                                                            ...
                                                            @elseif ($deposit->payment->type == 'e_wallet')
                                                                @if ($deposit->is_paid == 0)
                                                                    <b>Klik tombol dibawah untuk melakukan pembayaran</b>
                                                                    <br />
                                                                    <a href="{{ $paymentCheckout->data->checkout_url }}" target="_blank" class="btn btn-md btn-success font-weight-bold text-uppercase"> Bayar</a>
                                                        ...
                                                        @endif
                                                    @else
                                                        @if ($deposit->payment->is_qrcode)
                                                        {!! convertString($deposit->payment->information, $deposit->amount) !!}
                                                        <br />
                                                        SCAN QRCODE DI BAWAH INI !!!
                                                        <br />
                                                        <img src="{{ asset(config('constants.options.asset_img_qr_code') . $deposit->payment->qrcode) }}"
