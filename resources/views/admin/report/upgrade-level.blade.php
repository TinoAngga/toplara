...
<div class="row">
    ...
    <div class="col-md-12 col-lg-12 col-xl-12">
        <div class="card m-b-30 shadow">
            <div class="card-header bg-primary">
                <h5 class="card-title mb-0 text-white font-weight-bold"><i class="mdi mdi-account-multiple"></i> Peningkatan Pengguna</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12 mb-2 mt-2">
                        <div class="card shadow">
                            <div class="card-header bg-primary">
                                <h5 class="card-title text-white mb-0 font-weight-bold"> Total Semua Peningkatan Pengguna</h5>
                            </div>
                            <div class="card-body">
                                <h5 class="mb-0 loader" id="upgrade-level-count-all">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                                <br>
                                <h5 class="text-right mb-0 loader" id="upgrade-level-sum-all">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                        ...
                        <div class="card shadow">
                            <div class="card-header {{ getBgStatus($value) }}">
                                <h5 class="card-title text-white mb-0 font-weight-bold"><i
                                        class="mdi mdi-account-multiple"></i> {{ strtoupper($value) }}</h5>
                            </div>
                            <div class="card-body">
                                <h5 class="mb-0 loader" id="upgrade-level-status-count-{{ $value }}">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                                <br>
                                <h5 class="text-right mb-0 loader" id="upgrade-level-status-sum-{{ $value }}">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
...
<script>
    async function filter() {
        ...
        try {
            ...
            $.each(result, function(key, value){
                $('#upgrade-level-count-' + key).html(`${currency(value.count)} Peningkatan Level`);
                $('#upgrade-level-sum-'+ key).html(`Rp ${currency(value.sum ?? 0)}`);
            });
            $.each(result.status, function(key, value){
                $('#upgrade-level-status-count-'+ key).html(`${currency(value.count)} Peningkatan Level`);
                $('#upgrade-level-status-sum-'+ key).html(`Rp ${currency(value.sum ?? 0)}`);
            });
        } catch (error) {
            console.log(error);
        }
    ...
    async function getWidget(){
        let data = await $.getJSON('{{ route('admin.report.upgrade-level', ['widget' => 1]) }}');
