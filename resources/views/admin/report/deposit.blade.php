...
<div class="row">
    ...
    <div class="col-md-12 col-lg-12 col-xl-12">
        <div class="card m-b-30 shadow">
            <div class="card-header bg-primary">
                <h5 class="card-title mb-0 text-white font-weight-bold"><i class="mdi mdi-credit-card"></i> Deposit</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12 mb-2 mt-2">
                        <div class="card shadow">
                            <div class="card-header bg-primary">
                                <h5 class="card-title text-white mb-0 font-weight-bold"> Total Semua Deposit</h5>
                            </div>
                            <div class="card-body">
                                <h5 class="mb-0 loader" id="deposit-count-all">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                                <br>
                                <h5 class="text-right mb-0 loader" id="deposit-sum-all">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                    ...
                    <div class="col-lg-12 col-md-6 col-xl-4 mb-2 mt-2">
                        <div class="card shadow">
                            ...
                            <div class="card-body">
                                <h5 class="mb-0 loader" id="deposit-status-count-{{ $value }}">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
                                <br>
                                <h5 class="text-right mb-0 loader" id="deposit-status-sum-{{ $value }}">
                                    <div class="spinner-border" role="status">
                                        <span class="sr-only">Loading...</span>
                                    </div>
                                </h5>
...
<script>
    async function filter() {
        ...
        try {
            ...
            $.each(result, function(key, value){
                $('#deposit-count-' + key).html(`${currency(value.count)} Deposit`);
                $('#deposit-sum-'+ key).html(`Rp ${currency(value.sum ?? 0)}`);
            });
            $.each(result.status, function(key, value){
                $('#deposit-status-count-'+ key).html(`${currency(value.count)} Deposit`);
                $('#deposit-status-sum-'+ key).html(`Rp ${currency(value.sum ?? 0)}`);
            });
        } catch (error) {
            console.log(error);
        }
    ...
    async function getWidget(){
        let data = await $.getJSON('{{ route('admin.report.deposit', ['widget' => 1]) }}');
