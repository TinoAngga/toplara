...
<a href="javascript:;" onclick="deleteData(this, {{ $id }}, '{{ $title }}', '{{ route('admin.' .request()->segment(2). '.destroy', $id) }}', '<br /><small>jika Anda menghapus <b>{{ $title }}</b>, maka data yang bersangkutan akan ikut terhapus.')" class="badge badge-danger badge-sm" data-toggle="tooltip" title="Hapus">
