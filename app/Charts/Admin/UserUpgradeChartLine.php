...
class UserUpgradeChartLine
{
    ...
    public function build($request): \ArielMejiaDev\LarapexCharts\LineChart
    {
        ...
        foreach ($chart_item as $key => $value) {
            ...
            $chart['all'][] = \DB::table('user_upgrades')
                            ->select(\DB::raw('COUNT(id) AS count'))
                            ->whereDate('created_at', $value)
                            ->first()->count;
            $chart['pending'][] = \DB::table('user_upgrades')
                            ->select(\DB::raw('COUNT(id) AS count'))
                            ->where('status', 'pending')
                            ->whereDate('created_at', $value)
                            ->first()->count;
            $chart['sukses'][] = \DB::table('user_upgrades')
                            ->where('status', 'sukses')
                            ->select(\DB::raw('COUNT(id) AS count'))
                            ->whereDate('created_at', $value)
                            ->first()->count;
            $chart['gagal'][] = \DB::table('user_upgrades')
                            ->where('status', 'gagal')
                            ->select(\DB::raw('COUNT(id) AS count'))
                            ->whereDate('created_at', $value)
                            ->first()->count;
            $chart['kadaluarsa'][] = \DB::table('user_upgrades')
